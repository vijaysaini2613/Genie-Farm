...
// Resolved database path (writes to customer-app/db.json)
...
export async function GET(request: Request) {
  return runLocked(async () => {
    ...
    // If Supabase is configured, read directly from Supabase tables
    if (supabase) {
      try {
        switch (type) {
          case 'products': {
...
export async function POST(request: Request) {
  return runLocked(async () => {
    try {
      ...
      // If Supabase is configured, write directly to Supabase tables
      if (supabase) {
        try {
          switch (action) {
            case 'log': {
